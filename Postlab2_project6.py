...
class Player(object):
    def __init__(self):
        ...
        (i.e the frst sum of dice vales to match in order to win), roll count stores the count of rolls in the current game"""
    ...
    self.rolls = (v1, v2)
    sum = v1 + v2
    """If the current roll is the first roll of the game, game logic for first roll
    ...
    """if the current roll is not the first roll of the game, the player instance will
    have an initialSum(or 'Point' as per the game terminology) value assigned in the first roll and uses it in
    ...
    if (self.atStartUp):
      ...
      # it is clear that first roll has already occured therefore we do have a initialSum to match for.
        if sum == 7:
         self.loser = True
